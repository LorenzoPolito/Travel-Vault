---
type: knowledge-base
domain: travel/japan
category: itineraries
language: it
last_updated: 2026-09-03
destination: Japan
tags: [japan, itineraries, tokyo, kyoto, osaka, hiroshima, planning]
source_files:
  - Itinerari/Japan/
ai_notes: "File modulare Giappone. Nessun itinerario è definitivo — tutti sono varianti di lavoro."
total_itineraries: 19
---

# Itinerari — Giappone

## Come Aggiornare

> Quando crei un nuovo itinerario in `Itinerari/Japan/`, aggiungi una riga alla tabella "Itinerari Principali" e aggiorna `total_itineraries` nel frontmatter.

---

## Stato Generale

Viaggio **attivo A**: **Giappone, 23 Ott (partenza) — 6 Nov 2026** (15gg/14notti), arrivo 24 a Izumisano. Itinerario in `Itinerari/Japan/2026/Itinerario-Giappone-23ott-6nov2026.md`.

> ⚠️ **Variante B (27 ott — 11 nov)** rimossa: file eliminato, non più considerata (voli prenotati per la variante A).

> ✅ **Fact-check 11/08/2026:** prezzi/orari/tariffe verificati su fonti ufficiali (JR West, teamlab.art, todaiji.or.jp, japan-guide, xe.com). **Nishiki Market NON è chiuso di domenica** (correzione). Prezzi aggiornati: TeamLab ¥3.600, Skytree ¥2.400, Shibuya Sky ¥2.700, pass 17.000 ¥, Fuji Excursion ¥4.200.
> 🎡 **Aggiunto (11/08/26):** **PokéPark KANTO** (primo parco Pokémon permanente, Yomiuriland, aperto feb 2026) + **TeamLab** accorpati al **4 nov**; il **5 nov** è ultimo giorno libero senza attrazioni. **Fuji day trip rimosso del tutto.**
> 🧘 **Aggiornamenti 03/09/26:** Umeda Sky rimosso dal Giorno 3 (giornata tranquilla in zona Minami) + **Nipponbashi Den Den Town** (⭐ Lorenzo, opzionale) · ⛔ **PokéPark KANTO RIMOSSO (lotteria non vinta)** → il 4 nov = **TeamLab (slot pomeriggio) + Odaiba al tramonto**. Budget aggiornato a **~2.828 €/persona**.

---

## Organizzazione nel Vault

```
Itinerari/Japan/
├── 2026/                         ← Itinerari principali (dettagliati)
│   ├── Itinerario-Giappone-23ott-6nov2026.md      ← Variante A (23ott-6nov) — ATTIVO
├── Esterni/
│   ├── Itinerario Ossama Valentina.md
│   └── Rail Adventure SiVola.it (14 gg).md
└── Solo con i luoghi/
    ├── 7 giorni/    (×2)
    ├── 10 giorni/   (×2)
    ├── 11 giorni/   (×2)         ← Incl. versione dic 2025 posticipata
    ├── 12 giorni/   (×1)
    ├── 14 giorni/   (×9)         ← Varie combinazioni
    ├── 15 giorni/   (×1)
    └── 16 giorni/   (×1)
```

---

## Itinerario Principale — Variante A (23 Ott partenza — 6 Nov 2026)

**Path:** `Itinerari/Japan/2026/Itinerario-Giappone-23ott-6nov2026.md`

**Persone:** Lorenzo, Davide, Rebecca

| Tappa | Date | Notti |
| --- | --- | --- |
| Partenza Italia (volo notturno) | 23 Ott | — |
| Izumisano (KIX, notte d'arrivo) | 24 Ott | 1 |
| Osaka (incl. USJ) + day trip Hiroshima/Miyajima | 25–29 Ott | 4 |
| Nara (half day) → Kyoto | 29 Ott | — |
| Kyoto | 29 Ott – 1 Nov | 3 |
| Tokyo (TeamLab + Odaiba il 4 nov) | 1–6 Nov | 5 |
| Partenza (volo MU da HND) | 6 Nov | — |

**Caratteristiche chiave:**
- Orari al minuto, livello difficoltà fisica (1-4), indicazioni mezzo di trasporto
- ✅ **VOLI PRENOTATI** (09/08/26): China Eastern open-jaw 1.096,33 €/pax — FCO→KIX 23/10 21:10, HND→FCO 6/11 08:40
- Budget dettagliato: ~2.828 €/persona (voli inclusi)
- JR Pass nazionale NON consigliato → JR Kansai-Hiroshima 5gg + biglietto singolo Kyoto→Tokyo
- 1ª notte a Izumisano (KIX) — KURA Hotel, per arrivo 24 sera

---

## Tabella Itinerari (14 giorni)

| Nome file (abbreviato) | Percorso | Note |
| --- | --- | --- |
| **Itinerario-Giappone-23ott-6nov2026** | O→H→N→K→T | **Variante A** — 23ott-6nov, ATTIVO (voli prenotati) |
| Tokyo-Kyoto-Osaka-Hiroshima-Tokyo (15feb-1mar) | T→K→O→H→K→T | Variante Feb 2026 (archiviata) |
| Osaka-Hiroshima-Kyoto-Tokyo 22 Ott (15gg) | O→H→K→T | Include Miyajima |
| Osaka-Hiroshima-Myajima-Kyoto-Tokyo | O→H→M→K→T | Osaka-first |
| Osaka-Myajima-Kyoto-Tokyo | O→M→K→T | Senza Hiroshima separata |
| Osaka-Kyoto-Tokyo dist. based | O→K→T | Ottimizzato distanze |
| Tokyo-Kyoto-Hiroshima-Tokyo dist. based | T→K→H→T | Distance-based |
| Tokyo-Kyoto-Tokyo dist. based 1 | T→K→T | Solo Tokyo e Kyoto |
| Tokyo-Osaka-Kyoto-Tokyo dist. based | T→O→K→T | Osaka centrale |

---

## Itinerario Dicembre 2025 (posticipato/archiviato)

Budget:
| Voce | Stima |
| --- | --- |
| Voli + bagagli | 900 € |
| Notti (11 × ~35€) | 385 € |
| Trasporti | 365 € |
| Attività | 200 € |
| Pasti (2×11gg) | 550 € |
| Gadget/souvenir | 200 € |
| Imprevisti | 400 € |
| **TOTALE** | **~3.000 €** |

---

## Itinerari Esterni di Riferimento

| File | Percorso | Note |
| --- | --- | --- |
| Ossama & Valentina | Tokyo→Kyoto→Kobe→Hiroshima | Include Kobe e Museo Mazda |
| Rail Adventure SiVola.it | 14gg da tour operator | — |

---

## Tempi di Percorrenza Chiave

| Tratta | Mezzo | Durata |
| --- | --- | --- |
| Osaka → Hiroshima | Shinkansen Sakura | ~1h 30min |
| Osaka → Nara | Kintetsu Line | ~35min |
| Nara → Kyoto | JR Nara Line | ~45min |
| Kyoto → Tokyo | Shinkansen Hikari | ~2h 15min |
| Tokyo → Kamakura | JR Yokosuka Line | ~55min |
| Tokyo → Fujiyoshida | Fuji Excursion | ~1h 45min |
| KIX → Osaka centro | Haruka Express | ~50min |
| Tokyo → NRT | Narita Express | ~60min |

---

## Note per Agenti AI

- Nuovo itinerario → `Itinerari/Japan/<sottocartella>/NomeFile.md`
- Template Kanban pasti → `Itinerario Kanban Template.md` nella root
- Per il viaggio Ott 2026: **non serve il JR Pass nazionale** — usa JR Kansai-Hiroshima Area Pass (**17.000 ¥, attivare il 28 ott** — giorno di Hiroshima; valido 28 ott–1 nov)
- ✅ Voli prenotati (China Eastern, 1.096 €/pax) · ✅ Hotel prenotati
- Attività rimosse dal piano: Mario Kart Ride, Pokémon Café, Maid café
